...
function writeManagedObject(obj: ManagedObject) {
  ...
  for (const method of obj.methods) {
    ...
    {
        await this.Session.EamClient!.${methodName}(${args.join(", ")});
    }`;
      // END
    } else {
    ...
    {
        var res = await this.Session.EamClient!.${methodName}(${args.join(", ")});
    ...
    {
        return await this.Session.EamClient!.${methodName}(${args.join(", ")});
    }`;
        // END
      }
    }
  ...
  for (const param of method.parameters) {
    ...
    if (!param.mandatory && csStructureTypes.includes(ty)) {
      ...
      if (param.ty.remote == "ManagedObjectReference") {
        ...
        args.push(`${argName}${q}.EamReference`);
      } else if (param.ty.remote == "ManagedObjectReference[]") {
        const q = param.mandatory ? "" : "?";
        args.push(`${argName}${q}.Select(m => m.EamReference).ToArray()`);
      } else {
...
for (const obj of objs) {
  if (!obj.name.startsWith('Eam') && !classNames.includes(obj.name)) {
    continue;
  }
